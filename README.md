...
The `fxp_reference_writer` target **always** defines `FXP_PORTABLE_ONLY` and
`FXP_DISABLE_SSE2`, regardless of the rest of the build's settings. It generates
the frozen file using standard C++ scalar arithmetic. The SSE2 batch tests read
the frozen addition/subtraction results and compare their output directly with
them; SSE2 never generates the reference file.

The `compile_reject_*` tests build intentionally invalid targets through the
main project's configured compiler and generator. A valid integer/fixed-point
control must build first, and rejection tests share a CTest resource lock so
parallel test execution does not launch competing builds in the same directory.
The invalid targets are excluded from normal builds. After changing CMake files
or a VS Code CMake kit, configure/build the main project and refresh its tests.
